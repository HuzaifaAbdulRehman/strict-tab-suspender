...
4. Inspect `package/strict-tab-suspender-0.3.0.zip`, its `.sha256`, `.cdx.json`, and `.contents.txt`. The archive must contain only reviewed built assets; required permissions must be exactly `alarms`, `storage`, and `tabs`, with no optional/host permissions, remote URLs/code, source, tests, docs, or package metadata.
